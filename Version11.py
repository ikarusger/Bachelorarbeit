...
##neu
        ...
        vertices_xy = {(round(v[0], 5), round(v[1], 5)) for v in m_rot.vertices}
        triangles_xy = [
            (
    ...
    if positionen_koordinaten:   # Position abfragen und Koordinatne der Ecken ausgeben 
        ...
        if isinstance(duse, list):
            ...
            triangles_xy = pos.get("triangles_xy", [])  #neu
            ...
            if triangles_xy:
                ...
                for d, dx, dy, dz in duse:
                    ...
                    if transform is not None:
                        p_h = transform @ p_h
                    p_xy = (float(p_h[0]), float(p_h[1]))
                    if any(point_in_triangle_2d(p_xy, tri) for tri in triangles_xy):
                        print(f"  {d.name}: x,y={p_xy}")
            else:
                print("Keine Mesh-Dreiecks-Daten fuer diese Position.")  #neu
