...
if __name__ == "__main__": 
    ...
    finalkippachse = None
    if kipp_in == "u_z_p":  #Funktioniert nur für geschlossene Quader
        min_x = min(round(e[0], 5) for e in pos["ecken"])  #Der kleinste x-Wert alle Koordinaten wird gesucht
        print("minx",min_x)
        min_y = min(round(e[1], 5) for e in pos["ecken"])  #Der kleinste y-Wert aller Koordinaten wird gesucht
        print("miny",min_y)
        for idx, ((a, b), (ia, ib)) in enumerate(zip(pos["kanten_coords"], pos["kanten"])): #Geht alle Kanten durch und speicher die Kante mit den kleinsten x und y Werten unter finalkippachse ab
            ...
            if ax == min_x and ay == min_y and bx == min_x and by == min_y: #Vergleicht die Kantenpunkte mit den kleinsten x und y Werte aller Koordinaten
                finalkippachse = {
                    ...
                    "a": tuple(np.round(a, 5)), #Koordinaten runden
                    "b": tuple(np.round(b, 5)),
                    "key": (round(min_x, 5), round(min_y, 5)),
                }
                print(finalkippachse)
                break          #Beendet die Schleife, wenn die Kante gefundne wurde
       
        else:
            print("Kippachse u_z_p: keine Kante mit min x und min y gefunden")
