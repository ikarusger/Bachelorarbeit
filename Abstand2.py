...
def bestimme_zielpunkt(kipp_in: str, kante_x_laenge: float, kante_z_laenge: float) -> np.ndarray:  # kante_x_laenge ist die Länge der hinteren rechten unteren Kante, die parallel zur x Kante verläuft
    zielpunkte_pro_kipp: Dict[str, Vec3] = {
        "o_x_p": (0, 0.0, 0.0),
        "o_x_n": (0, 0.0, 0.0),

        "u_x_p": (-25.0+(kante_x_laenge/2), 0.0, 0.0),
        "u_x_n": (-25.0+(kante_x_laenge/2), 0.0, 0.0),

        "u_z_p": (0.0, 0.0, 40.0-(kante_z_laenge/2)),
        "u_z_n": (0.0, 0.0, 40.0-(kante_z_laenge/2)),
    }
    ...
    with csv_path.open("w", newline="", encoding="utf-8") as csv_file: #Öffnet die Datei und sorgt dafür, dass sie am Ende wieder geschlossen wird
        ...
        for pfad_teil in pfade: #Schleift durch alle Pfade
            ...
            for position_id, (R3, label) in enumerate(rotations, start=1):  #Transformation beginnt erst wird das Mesh pro Position orientiert dann verschoben
                ...
                schwerpunkt_rot = (T @ schwerpunkt_h)[:3]
            ...
            for pos_id in range(1, len(positionen_koordinaten) + 1):
                ...
                for kipp_in in kipp_inputs: #Schleife, die durch alle kipp_inputs schleift
                    ...
                    # Mesh wird erst hier (nach Auswahl von kipp_in) auf den Zielpunkt verschoben.
                    T_pos = pos["T"]
                    m_sel = m.copy()
                    m_sel.apply_transform(T_pos)
                    ...
                    zielpunkt = bestimme_zielpunkt(kipp_in, kante_x_laenge, kante_z_laenge)
                    e5 = np.array([maxx, miny, minz], dtype=float)
                    T_shift = np.eye(4)
                    T_shift[:3, 3] = zielpunkt - e5
                    T_total = T_shift @ T_pos
                    ...
                    T_shift_target[:3, 3] = zielpunkt - e5_target
                    T_total_target = T_shift_target @ T_target
