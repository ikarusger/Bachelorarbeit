        ...
        for pfad_teil in pfade: #Schleift durch alle Pfade
            ...
            for pos_id in range(1, len(positionen_koordinaten) + 1):
                ...
                for kipp_in in kipp_inputs: #Schleife, die durch alle kipp_inputs schleift
                    ...
                    #Berechnung der Kippachse der Zielorientierung
                    #m_target.apply_transform(T_shift_target) #Verschiebt das bereits rotierte Mesh auf den Ursprung, kann nicht direkt auf T_to_ziel_target verschoben werden  debug
                    #m_target.apply_transform(T_to_ziel_target) #Verschiebt das Mesh vom Ursprung auf den Zielpunkt
                    #echte_kanten_und_ecken_ziel = echte_kanten_und_ecken(m_target, angle_deg=90.0, tol_deg=1.0) #Bestimmt die echten Kanten
                    #pos_transformiert_ziel = {
                    #    "ecken": echte_kanten_und_ecken_ziel["corner_coords"], #Erstellt ein Dictionary das die Kanten der Zielgeometrie enthalten
                    #}
                    #finalkippachse_ziel = find_kippachse(pos_transformiert_ziel, kipp_in) #Ruft find_kipppachse auf und Speichert die finale Kippachse ab 
                    ...
                    for finalduse in finaldusen: #Schleife durch alle möglichen Düsen
                        ...
                        z_distanz_duese_schwerpunkt = abs(z_duese - z_schwerpunkt)

                        if kipp_in in {"u_x_n"}:
                            distanz = z_distanz_duese_schwerpunkt
                    ...
                    #Berechnung von OD
                        ...
                        # Neue Berechnung fuer die Zielorientierung (Schwerpunkt2 zur Ziel-Kippachse)
                        
                        #ka_ziel = np.array(finalkippachse_ziel["a"], dtype=float) #Punkt a der Ziel Kippachse
                        #kb_ziel = np.array(finalkippachse_ziel["b"], dtype=float) #Punkt b der Ziel Kippachse
                        #kab_ziel = kb_ziel - ka_ziel   #Bildet den Richtungsvektor der Ziel-Kippachse 
                        #kante_len_ziel = np.linalg.norm(kab_ziel) #Berechnet die Länge des Richtungsvektors
                        #norm_kab_ziel = kab_ziel / kante_len_ziel #Normiert den Richtungsvektor auf 1
                        #schwerpunkt2_ka = schwerpunkt_pos2 - ka_ziel #Berechnet den Vektor von Kippachsenpunkt zum Schwerpunkt.  Der Kippachsenpunkt ist hier ka_ziel, ich könnte aber auch kb_ziel verwenden. Es muss nur ein Punkt auf der Ziel Kippachse sein.
                        #schwerpunkt2_ka_senk = schwerpunkt2_ka - np.dot(schwerpunkt2_ka, norm_kab_ziel) * norm_kab_ziel # Zerlegt schwerpunkt2_ka in einen parallelen und einen senkrechten Anteil. Hier bleibt der senkrechte Anteil zur Kippachse übrig.
                        #schwerpunkt2_kippachse_abstand = float(np.linalg.norm(schwerpunkt2_ka_senk)) #Berechnet die Länge des Senkrechten Vektors. Damit wird der kürzeste Abstand vom Schwerpunkt zur Ziel Kippachse berechnet.
                        #h_ziel = abs(round(float(schwerpunkt2_kippachse_abstand) - float(schwerpunkt_pos2[1]), 5)) #Berechnet den Hoehenunterschied  des Schwerpunktes2 und des Abstand zwischen Schwerpunkt2 und Kippachse
                        #print("Abstand Schwerpunkt2 zur Ziel Kippachse:", schwerpunkt2_kippachse_abstand) #Debug
                        #print("Hoehenunterschied Zielorientierung:", h_ziel)
                        ...
                        #h_gesamt= h + h_ziel
                        #print("Ausgangshoehe:",h)
                        #print("Zusatzhoehe:",h_ziel)
                        #print("Gesamthöhe:",h_gesamt)
                        #Berechnung der Impulskraft mit dem neuen Höhenunterschied + Höhenunterschied der Zielposition (Maximale Impulskraft)
                        #F_ziel_t = math.sqrt((mass * g * (abs(h_gesamt)) * 2 * math.pow(I_kipp, 2))  / (I_kipp * math.pow(t_ziel, 2) * math.pow(OD, 2))) # F in kg/mms2
                        #F_ziel_t = round(F_ziel_t / 1000, 5)  # F in kg/ms2 (N)
                        #print(f"Kraft bei t={t:.2f}s: ", F_ziel_t, ("N"))
