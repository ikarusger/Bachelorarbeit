...
def ray_intersects_any(triangles: np.ndarray, origin: np.ndarray, direction: np.ndarray, eps: float = 1e-9) -> bool:
    # Moller-Trumbore ray/triangle test without rtree dependency.
    if triangles.size == 0:
        return False
    v0 = triangles[:, 0]
    v1 = triangles[:, 1]
    ...
    hit = mask & (u >= 0.0) & (v >= 0.0) & ((u + v) <= 1.0) & (t > eps)
...
if __name__ == "__main__": 
    ...
    for d, dx, dy, dz in funkduse:
        ...
        if d.kraft == "u":
            ...
            hit = ray_intersects_any(triangles_sel, origin, direction) #Prüft, ob der Strahl irgendein Dreieck der Oberfläche trifft
            print("    xz_surface:", hit)
        elif d.kraft == "o":
            origin = np.array([d.pos[0], d.pos[1], z_min], dtype=float)
            direction = np.array([0.0, 0.0, 1.0], dtype=float)
            hit = ray_intersects_any(triangles_sel, origin, direction)
            print("    xy_surface:", hit)
