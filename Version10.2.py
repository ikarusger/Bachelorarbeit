...
duesen: Dict[str, Duse] = {   #   x    y    z
    ...
    "DuseO1": Duse("DuseO1", (-10.0, 10.0, 0.0), "o"),  # DuseO O ist keine Null sonder ein groÃŸes o
...
def load_mesh(path: str) -> trimesh.Trimesh: #LÃ¤dt das Trimesh und gibt es aus
...
def rot_z_90k(k: int) -> np.ndarray:
    """Drehung um z-Achse: k * 90Ã‚Â° als 3x3 Matrix."""
    k = k % 4 # stellt sicher, dass auch wenn Werte Ã¼ber 4 eingeben werden, es funktioniert. Dies benÃ¶tige ich, wenn ich zum Bsp. Drehungen addiere.
    if k == 0:
...
def kipp_in_to_rot(kipp_in: str) -> np.ndarray | None: #Rotation um den Schwerpunkt2 Gibt die Rotation zurÃ¼ck
    if kipp_in == "o_x_p":
        return rot_x_90k(1)
    if kipp_in == "o_x_n":
        return rot_x_90k(3)
...
def positionieren24() -> list[tuple[np.ndarray, str]]: #Erstellt die Position
    ...
    bases = [    # Liste mit Tupeln     alle 6 FlÃ¤chen liegen einmal unten AusgangsflÃ¤che, die im nÃ¤chsten Schritt dann um die zAchse gedreht werden. So erhalte ich alle Positionen
    ...
    for base_label, base in bases:
        for k in range(4):
            ...
            z_deg = k * 90 #FÃ¼r das label
            label = f"{base_label}_z{z_deg}" #FÃ¼rd das label
...
#PrÃ¼ft ob es sich um eine echte geometrische Kante handelt, oder um eine vom Mesh erstellte Kante. Dies tritt z.B bei LÃ¶chern auf. Diese extra Kanten der LochÃ¶ffnung werden herausgerechnet, da disee nicht achsenparallel sind. Diese LÃ¶sung funktioniert nur bei KÃ¶rpern, wo alle Kanten Achsenparallel verlaufen.
def axis_richtung(vec: np.ndarray, rel_tol: float, xyz_tol: float) -> int | None: #PrÃ¼fst, ob ein Richtungsvektor nahezu achsenparallel ist, indem die grÃ¶ÃŸte Komponente als Hauptachse ausgewÃ¤hlt wird  und alle anderen Komponenten nur innerhalb einer Toleranz zulÃ¤sst sind Wenn das passt, gibst  die Achse (0=x, 1=y, 2=z) zurÃ¼ck, sonst None.
    xyz_v = np.abs(vec) # Nimmt den Betrag von x y z des Vektors ohne Vorzeichen 
    max_komp = float(np.max(xyz_v)) #Bestimmt die grÃ¶ÃŸte Kompnente also z.b x
    #if max_comp <= abs_tol: #Wenn der Wert null ist, ist keine Richtung definierrbar 
       # return None # Dann bricht die funktion ab, da es keinen gÃ¼ltigen Achsenvektor gibt 
    axis = int(np.argmax(xyz_v)) #Die Achse mit der grÃ¶ÃŸten Komponente wird als Kandidat gewÃ¤hlt zum bsp x 
    tol = max(xyz_tol, rel_tol * max_komp) #toleranz fÃ¼r Nebenachsen, da stl Dateien oft kleine Rundungsfehler haben fÃ¼r y z
    for i in range(3):
        if i != axis and xyz_v[i] > tol:#Wenn der Wert auf einer Nebenachse grÃ¶ÃŸer ist als die Toleranz non   Wenn y<x dann non oder wenn z<x dann non, da x die Hautpachse ist
            return None
    return axis
# OPTION B END

#Berechnen die echten Kanten dadurch, dass es nur Kanten als echt befindet die 90Â° zu einer anderen Kante stehen
def echte_kanten_und_ecken(
    ...
    min_corner_axes: int = 3, #Wie viele Achsenrichtungen an einer Ecke zusammenkommen mÃ¼ssen. FÃ¼r eine Ecke 3
) -> dict:
    fa_edges = mesh.face_adjacency_edges #Hohlt alle Kanten, die zur benachtbarten FlÃ¤che gehÃ¶ren
    fa_angles = mesh.face_adjacency_angles #Fragt die Winekl zwischen diesen benachbarten FlÃ¤chen ab
    ...
    diff = np.abs(np.degrees(fa_angles) - angle_deg) #Berechen den Abstand des FlÃ¤chenwinkel zum Zielwinkel 90
    edge90 = fa_edges[diff <= tol_deg] #Filter nur Kanten, deren Winkel im Toleranzbereich liegen.
    boundary = mesh.edges_boundary if hasattr(mesh, "edges_boundary") else np.empty((0, 2), dtype=int) #Fragt Randkanten des Meshs ab, da sie keine NachbarflÃ¤che haben und damit keinen Winkelwert face_adjacency_angles
    if boundary is not None and len(boundary) > 0: #PrÃ¼ft ob es Randkanten gibt
        edge90 = np.vstack([edge90, boundary]) #FÃ¼gt die Rankanten zu edge90 hinzu
    if len(edge90) == 0:
        return {"edge_indices": np.empty((0, 2), dtype=int), "corner_indices": np.array([], dtype=int),
                "edge_coords": [], "corner_coords": []} #Wenn es keine edge90, also 90Â°Kante,  gibt bircht die Funktion ab.
    edge90 = np.unique(np.sort(edge90, axis=1), axis=0)

    # OPTION B START: axis-parallel edge filtering
    #Hier wereden nur die achsenparallelen Kanten aus den 90Â°Kanten edge90 gefiltert
    axis_edges = [] #Liste fÃ¼r alle achsenparallelen Kanten
    axis_dirs = [] #Speicher, in welche Richtung jede gefundene Kante zeigt (0/1/2) x y z 
    for a, b in edge90: #Schleife Ã¼r alle 90Â° Kanten
        ...
        if axis is not None: #Wenn die achsenparallel
            axis_edges.append((a, b)) #Kanten hinzufÃ¼gen
    ...
    axes_by_vertex = [set() for _ in range(len(mesh.vertices))] #FÃ¼r jeden Eckpunt/Vertex wird ein Set erstellt, in das die Achsenrichtungen der Kanten eingetragen werden, die an diesem Eckpunkt/Vertex hÃ¤ngen
    for (a, b), axis in zip(axis_edges, axis_dirs): #Schleife Ã¼ber Kanten und Achsen 
        ...
        #Das brauche ich, da ich ja spÃ¤ter abfrage wie viel Achsen an einem Eckpunkt/vertex zusammenkommen. Wenn es mehr als 3 sind ist es ein Eckpunkt
    ...
    return { #Die Ergebnisse werden ins Dict zurÃ¼ckgegeben
...
#Debug Diagnose des Meshs             PrÃ¼ft, ob OptionB das Probelem mit den LÃ¶chern im Mesh gelÃ¶st hat
...
#Bestimmt welche DÃ¼se zum Kippen zu gebrauchen ist
def kipp_duse(kipp_in: str, schwerpunkt: Vec3) -> list[tuple[Duse, float, float | None, float | None]] | None:    #KippmÃ¶glichkeiten   schwerpunkt wird als transofmirten Vector ausgegeben x, y, z
    if kipp_in == "o_x_p":
        funk_duesen = []                    # Liste mit DÃ¼sen die zum Kippen verwendet werden kÃ¶nnen d: das Duse-Objekt  dx: Abstand in x zum Schwerpunkt   dz: Abstand in z um Schwerpunkt
        for d in duesen.values():
            if d.kraft == "o" and d.pos[1] > schwerpunkt[1]: 
                dx = abs(d.pos[0] - schwerpunkt[0])     #Abstand zwischen DÃ¼se und Schwerpunkt        abs() gibt den Betrag aus
        ...
        funk_duesen = neue_liste  # Ãœberschreibt alte Liste
        return funk_duesen
    ...
    if kipp_in == "u_x_n":
        funk_duesen = []                    # Liste mit DÃ¼sen die zum Kippen verwendet werden kÃ¶nnen d: das Duse-Objekt  dx: Abstand in x zum Schwerpunkt   dz: Abstand in z um Schwerpunkt
        for d in duesen.values():
            if d.kraft == "u" and d.pos[2] > schwerpunkt[2]: # Nur DÃ¼sen mit u und der z Wert der DÃ¼sen muss grÃ¶ÃŸer sein als der z Wert des Schwerpunkts
                dx = abs(d.pos[0] - schwerpunkt[0])     #Abstand zwischen DÃ¼se und Schwerpunkt        abs() gibt den Betrag aus
                dz = abs(d.pos[2] - schwerpunkt[2])   # 0 = x, 1 = y, 2 = z
                dy = None
                funk_duesen.append((d, dx,dy, dz))
        neue_liste = []
        for d, dx,dy, dz in funk_duesen:  # FÃ¼gt nur Objekte zur neuen Liste hinzu wenn x<z 
            if dx < dz:
                neue_liste.append((d, dx,dy, dz))  
        funk_duesen = neue_liste  # Ãœberschreibt alte Liste
        return funk_duesen
...
def find_kippachse(pos: dict, kipp_in: str) -> dict | None: #Die Kippkante wird anhand der echten Bauteilkanten bestimmt. Es kann auch eine neue Kippkanten anhand z.B. der kleinsten Punkte erstellt werden. Das ist notwendig wenn die Bauteilkante unterborchn wird.
    ...
    return { #Neue Kippkante wird aus den Bedingungen fÃ¼r a unb b aus den jeweiligen Eckpunkten bestimmt
        "edge_index": -1,
...
def create_nozzle_cylinders(positions: list[np.ndarray]) -> trimesh.Trimesh | None:  #Zylinder, die DÃ¼sen symbolisieren fÃ¼r die stl-Datei
    if not positions:
        return None
    spheres = []
...
if __name__ == "__main__": 
    ...
    echte_geo = echte_kanten_und_ecken(m, angle_deg=90.0, tol_deg=1.0)  #print fÃ¼r die Konsole der Ausgangskoordinaten nicht transformiert
    ...
    positionen_koordinaten = []      #Liste fÃ¼r alle 24 Positionen wird erstellt 

    Schwerpunkt = tuple(round(v, 5) for v in m.center_mass) # Ausgangsschwerpunkt wird berechnet 

    #Erstellt alle Meshs fÃ¼r alle Positionen
    ...
    for position_id, (R3, label) in enumerate(rotations, start=1):  #Transformation beginnt erst wird das Mesh pro Position orientiert dann verschoben
        ...
        m_ver_orie = m.copy()     # Kopie, Original bleibt unverÃ¤ndert
        ...
        # Speichert Transformation und Mesh-Daten fÃ¼r spÃ¤tere Ray-Test/Strahl 
        T_total = T_shift @ T
        bounds_sel = m_ver_orie.bounds
        y_min = bounds_sel[0][1] - 1.0 #Setzt start y Wert unterhalb des Meshes fÃ¼r den y-Strahl
        z_min = bounds_sel[0][2] - 1.0 #Setzt start z Wert unterhalb des meshs fÃ¼r den z-Strahl
        ...
        #echte enthÃ¤lt dann die verschobenen und orientierten Koordianten der Ecken und Kanten da in m_ver_orie die verschiebung und orientierung enthalten ist
        
        #Definierung des Dict
        positionen_koordinaten.append(  #FÃ¼r jede Position wird ein Dictionary angelget mit den relevanten Daten
            {  #benÃ¶tige ich, wenn ich auf z.B Eckkordinaten einer bestimmten Position zugreifen will
                ...
                # Daten fÃ¼r Ray-Tests auf der OberflÃ¤che der aktuellen Position
                "T_total": T_total,
                "y_min": y_min,
                "z_min": z_min,
            }
        ...
        #Legt Ordner fest fÃ¼r die exportierten stl-Dateien
    ...
    kipp_optionen = ["o_x_p", "o_x_n", "o_y_p", "o_y_n", "u_x_p", "u_x_n", "u_z_p", "u_z_n"]
    ergebnisse = []
    rho = 1.1e-6  # kg/mm^3
    t = 0.1 #s
    g = 9810 #mm/s2

    for pos in positionen_koordinaten:
        position_id = pos["position_id"]
        schwerpunkt_pos = pos["schwerpunkt"]
        T_total = pos["T_total"]
        y_min = pos["y_min"]
        z_min = pos["z_min"]
        ...
        m_eig = m_sel.mass_properties
        tm = m_eig["inertia"] * rho
        mass = m_eig["volume"] * rho

        rot_ma_akt_pos, _ = rotations[position_id - 1]

        for kipp_in in kipp_optionen:
            funkduse = kipp_duse(kipp_in, schwerpunkt_pos)
            if not funkduse:
                continue

            rot90 = kipp_in_to_rot(kipp_in)
            if rot90 is None:
                continue

            z_rot = rot90 @ rot_ma_akt_pos
            z_pos = None
            for idx, (R, _) in enumerate(rotations, start=1):
                if np.allclose(R, z_rot, atol=1e-6):
                    z_pos = idx
                    break
            if z_pos is None:
                continue

            schwerpunkt_pos2 = positionen_koordinaten[z_pos - 1]["schwerpunkt"]

            finalkippachse = find_kippachse(pos, kipp_in)
            if finalkippachse is None:
                continue

            ka = np.array(finalkippachse["a"], dtype=float)
            kb = np.array(finalkippachse["b"], dtype=float)
            kab = kb - ka
            kante_len1 = np.linalg.norm(kab)
            if kante_len1 == 0:
                continue

            e_k_ab = kab / kante_len1
            Iachse = float(e_k_ab @ tm @ e_k_ab)
            schwerpunkt_ka = schwerpunkt_pos - ka
            schwerpunkt_ka_senk = schwerpunkt_ka - np.dot(schwerpunkt_ka, e_k_ab) * e_k_ab
            schwerpunkt_ka_senk2 = np.dot(schwerpunkt_ka_senk, schwerpunkt_ka_senk)
            I_kipp = Iachse + mass * schwerpunkt_ka_senk2
            if I_kipp == 0:
                continue

            h = abs(round(schwerpunkt_pos2[1] - schwerpunkt_pos[1], 5))

            for d, dx, dy, dz in funkduse:
                if d.kraft == "u":
                    origin = np.array([d.pos[0], y_min, d.pos[2]], dtype=float)
                ...
                ds = np.array(closest, dtype=float)

                if kipp_in in {"o_x_p", "o_x_n"}:
                    OD = abs(ds[1] - ka[1])
                elif kipp_in in {"o_y_p", "o_y_n"}:
                    OD = abs(ds[0] - ka[0])
                else:
                    kante_len2 = np.dot(kab, kab)
                    if kante_len2 == 0:
                        continue
                    prof = np.dot(ds - ka, kab) / kante_len2
                    prof = float(np.clip(prof, 0.0, 1.0))
                    closest_axis = ka + prof * kab
                    OD = float(np.linalg.norm(ds - closest_axis))

                if OD == 0:
                    continue

                F = math.sqrt((mass * g * h * 2 * math.pow(I_kipp, 2))  / (I_kipp * math.pow(t, 2) * math.pow(OD, 2)))
                F = round(F / 1000, 5)  # F in kg/ms2 (N)
                ergebnisse.append((position_id, z_pos, kipp_in, d.name, F))
    ...
    with csv_path.open("w", newline="", encoding="utf-8") as csv_file:
        ...
        writer.writerow(["Ausgangsposition", "Position_nach_Kippen", "kipp_in", "finalduse", "Kippkraft"])
