...
if __name__ == "__main__": 
    ...
    with csv_path.open("w", newline="", encoding="utf-8") as csv_file: #Öffnet die Datei und sorgt dafür, dass sie am Ende wieder geschlossen wird
        ...
        for pfad_teil in pfade: #Schleift durch alle Pfade
            ...
            duse_u10 = np.array(duesen["DuseU10"].pos, dtype=float)
            ...
            for position_id, (R3, label) in enumerate(rotations, start=1):  #Transformation beginnt erst wird das Mesh pro Position orientiert dann verschoben
                ...
                # Verschiebung je Position:
                # Schwerpunkt in x/z auf DuseU10 legen und Bauteil in y auf Grundebene setzen.
                bounds = m_ver_orie.bounds
                miny = bounds[0][1] #Untere Ecke der Bounding box in y

                schwerpunkt_rot_only = (T @ np.array([*Schwerpunkt, 1.0], dtype=float))[:3]
                T_shift = np.eye(4)
                T_shift[:3, 3] = np.array([
                    duse_u10[0] - schwerpunkt_rot_only[0],  # Schwerpunkt-x auf U10-x
                    -miny,                                   # Unterkante auf y=0
                    duse_u10[2] - schwerpunkt_rot_only[2],  # Schwerpunkt-z auf U10-z
                ], dtype=float)
